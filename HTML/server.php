...
if (isset($_GET['action'])) {
    ...
    if ($action == 'register') {
        ...
        $colorblindness = $input['colorblindness'];
        $age = $input['age'];
        ...
        if ($result->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO users (username, password, email, colorblindness, age) VALUES (?, ?, ?, ?, ?)");
            $stmt->bind_param("sssss", $username, $password, $email, $colorblindness, $age);
    ...
    elseif ($action == 'update') {
        ...
        $email = $input['email']; // Obtém o email do input JSON
        $colorblindness = $input['colorblindness']; // Obtém a deficiência
        $age = $input['age']; // Obtém a idade

        // Atualiza os dados do usuário no banco de dados
        $stmt = $conn->prepare("UPDATE users SET password=?, email=?, colorblindness=?, age=? WHERE username=?");
        $stmt->bind_param("sssss", $password, $email, $username, $colorblindness, $age);
